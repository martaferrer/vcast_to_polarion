...
class PolarionGateway(base_gateway.BaseGateway):
    def __init__(self, **kwargs):
        ...
        self.trace_buffer = ''
        self.trace_to_stdout = False
        self.field_indexes = {}
        self.min_fields_needed = 0
        self.export_data = {}
        self.tc_export_data = {}
        self.dictionaryList = {}
    ...
    def cli_interactive_config(self, db_path, profile_name, mode):
      ...
      if ( mode == "connect" ):
        ...
        if "polarion_query_refinement" not in self.settings:
          self.settings["polarion_query_refinement"] = "status:(accepted active approved draft) AND author.id:$[user.id]"
        ...
        self._cli_prompt_setting_bool("Prepend Environment name to test case name?", "polarion_prepend_environment_name")
        result = True
      else:
    ...
    def _get_list_data_hardcoded(self):
      ...
      workItemTypeList = [businessWorkItemType, reqWorkItemType, testcaseWorkItemType, taskWorkItemType, changeReqWorkItemType, defectWorkItemType, riskWorkItemType]
    ...
    def _get_list_data_from_server(self):
      ...
      # For get_list_data
      self.dictionaryList = {'attributes':attributesList, 'work_item_types':workItemTypeList, 'testrun_template_types':testRunTemplateTypeList}
    ...
    def _write_requirement_data(self, item, out_file):
        ...
        out_file.write('    ' + xml.tostring(req_element) + '\n')


    def login_to_polarion(self):
        # Load settings, create clients, login to Polarion and prepare clients headers using the login response

        # Load settings.
        self._trace ("Settings: " + str(self.settings), 5)

        self.username           = self.settings.get('user','')
        self.password           = self.settings.get('password','')
        self.polarionWebServer  = self.settings.get('server','')
        self.projectID          = self.settings.get('project_id','')
        self.testRunID          = self.settings.get('polarion_test_run_id','')
        self.workItemType       = self.settings.get('polarion_import_work_item_type')
        self.testCaseType       = self.settings.get('polarion_export_work_item_type')
        self.queryRefinement    = self.settings.get('polarion_query_refinement')

        testCaseNoneAsPassed    = self.settings.get('polarion_test_case_none_as_passed')
        prependEnvironmentName  = self.settings.get('polarion_prepend_environment_name')

        if ( testCaseNoneAsPassed == '1'):
            self.treatTestCaseResultNoneAsPassed = True
        else:
            self.treatTestCaseResultNoneAsPassed = False

        if ( prependEnvironmentName == '1'):
            self.prependEnvToTestCaseName = True
        else:
            self.prependEnvToTestCaseName = False

        self._trace ("username:--" + self.username + "--", 3)
        #self._trace ("password:--" + self.password + "--", 3)
        self._trace ("polarionWebServer:--" + self.polarionWebServer + "--", 3)
        self._trace ("projectID:--" + self.projectID + "--", 3)
        self._trace ("testRunID:--" + self.testRunID + "--", 3)
    ...
    def start_fetch(self, output_file_path):

        if not self._checkAttributeMapping():
            return False

        self._trace ("Polarion requirements import started", 1)
        self._trace ("Polarion project ID: " + self.projectID, 3)
        self._trace ("Client service:" + self.polarionWebServer + "SessionWebService?wsdl", 3)
        self._trace ("Tracker service:" + self.polarionWebServer + "TrackerWebService?wsdl", 3)
        self._trace ("Attempting to establish SOAP connection to Polarion Server", 2)
        self._trace ("Please wait...", 1)
        ...
        # Append query refinement.
        if ( self.queryRefinement != ''):
            reqQueryString += ' AND ' + self.queryRefinement
        ...
        try:
            ...
            return False
        self._trace ("Requirements query result: " + str(reqString), 4)

        out_file = codecs.open(output_file_path, 'wb', 'utf-8')
        out_file.write('<req_data>\n')
        out_file.write('  <group name="[Polarion] ' + self.projectID + '">\n')

        nbrReq = 0
        # Iterate over reqString: A list of all requirements returned by Polarion.
        for item in reqString:
            nbrReq += 1
            self._write_requirement_data(item, out_file)

        if nbrReq > 0:
            self._trace (str(nbrReq) + " requirements imported.", 1)
        else:
    ...
    def start_export_old(self, log_path):
        # Initiate exporting requirement test data, logging status for inidividual
        # requirements to log_path.  Return whether the initiation was successful.

        self._trace ("Starting export", 1)
        if not self._checkExportFields():
            return False

        testCaseNameList = []  # List of testcase names that we already treated
        testCaseUriList = []   # Uri of the already treated test cases. Index in UriList = index in nameList

        # Get Requirements keys from VC
        ...
        for k in keys:
            # Get VC data for requirement
            # If multiple testcases verify one req VC provides one string with all test case names seperated by '\n'
            # Same applies to test results. Convert these strings to lists to allow easier access
            req_data = self.export_data[ k ]
            self._trace ("Processing req string: " + str(req_data), 4)
            wiID = req_data.get('key', '')
            self._trace ("Key: " + wiID, 4)
            self._trace ("Computing requirement " + wiID, 2)

            envNames = req_data.get('environment_name', '')
            listEnvNames = envNames.split('\n')
            self._trace ("Environment-name: " + envNames, 4)

            testNames = req_data.get('test_name', '')
            listTestNames = testNames.split('\n')
            self._trace ("test-name: " + testNames, 4)
            self._trace ("test-name-list: " + str(listTestNames), 4)

            testStatus = req_data.get('test_status', '')
            self._trace ("test-status: " + testStatus, 4)
            listTestStatus = testStatus.split('\n')
            self._trace ("test-status-list: " + str(listTestStatus), 4)

            if (len(listTestNames) != len (listTestStatus)) or (len(listTestNames) != len (listEnvNames)):
                self._trace ("Internal error: Nbr of test cases doesn't match nbr of test results or nbr of env for key " + wiID, 1)
                self._trace ("listTestName: " + str(listTestNames), 1)
                self._trace ("listTestStatus: " + str(listTestStatus), 1)
                self._trace ("listEnvNames: " + str(listEnvNames), 1)
                self._set_error_trace("Internal error: Nbr of test cases doesn't match nbr of test results or nbr of env for key " + wiID)
                return False
            ...
            for index, testName in enumerate(listTestNames):
                self._trace ("Computing test case " + listEnvNames[index] + ':' + testName + ':' + listTestStatus[index], 3)

                tcUri = ""
                testNameTitle = ""
                if self.prependEnvToTestCaseName:
                    testNameTitle = listEnvNames[index] + ':' + testName
                else:
                    testNameTitle = testName
                self._trace ("Test case name for Polarion update:" + str (testNameTitle), 3)
                self._trace ("testCaseNameList: " + str(testCaseNameList), 4)

                if ((testNameTitle not in testCaseNameList) and (len(testName) > 0)):
                    self._trace ("testCase Name not in set :" + str(testNameTitle), 3)
                    testCaseNameList.extend ([testNameTitle])
                    ...
                    if qResult == []:
                        ...
                        self._trace ("Test case not found in Polarion. Will create it", 3)
                        ...
                        try:
                            ...
                            self._trace ("Test case successfully created. Result: " + str(tcUri), 3)
                        except Exception, e:
                        ...
                        if len(qResult) == 1:
                            ...
                            self._trace ("One tescase found. Uri:" + tcUri, 2)
                        else:
                            self._set_error_trace("More than one tescases found for query " + str(tcQueryStrg))
                    ...
                    self._add_testRecord (testRun._uri, tcUri, listTestStatus[index], userUri)
                    # Add test case Uri to the local list
                    testCaseUriList.extend ([tcUri])
                else:
                    if len(testName) == 0:
                        self._trace ("Testcase name empty. Test cases removed from requirement " + str(wiID) + ". Please check links in Polarion", 1)
                    else:
                        self._trace ("Testcase already treated", 3)
                        # Get tcUri from List
                        testCaseNameIndex = testCaseNameList.index(testNameTitle)
                        tcUri = testCaseUriList[testCaseNameIndex]
                        self._trace ("Testcase index: " + str(testCaseNameIndex) + " Testcase Uri: " + str(tcUri), 3)

                if len(testName) > 0:
                    # Update the linkage to the requirement (verifies/is verified by)
                    self._add_linkItem (reqWi, tcUri)

                # Mark requirement as beeing exported
